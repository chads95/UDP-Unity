...
namespace Udp
{
    class cVMS_MsgHandler : MonoBehaviour
    {
        ...
        public struct PERIODIC_DEFN
        {
            ...
            {
                nMsgDefn = new byte[RC_MsgDefnsVMS.MAX_MSG_DATA_DEFNS + 1];
                diMsgFreq = new cDI_Byte( nData, nIdentifier );
                nMsgSkip = nSkip;
            }
        }

        public Dictionary<byte, PERIODIC_DEFN> m_dictPeriodic = new Dictionary<byte, PERIODIC_DEFN>();

        public cVMS_MsgHandler() { }
        ...
        public bool handleRxMessage( UdpClient sock, byte[] msg_buf, IPEndPoint client )
        {
            ...
            RC_MsgHelpers.formRC_RespHeader( msg_buf[(byte)RC_MsgDefnsVMS.MsgByte.MSG_BYTE_SUB_SYS],
                                             msg_buf[(byte)RC_MsgDefnsVMS.MsgByte.MSG_BYTE_SUB_SYS_INDEX],
                                             ref cReplyMsg, ref nReplyLength );

            // Calc the checksum and compare the the rx checksum
            byte uCalcChecksum = RC_MsgHelpers.calcRC_Checksum( msg_buf, nMsgLength - 1 );
            ...
            if ( uCalcChecksum != msg_buf[( nMsgLength - 1 )] )
            {
                ...
                cStatusByte |= RC_MsgDefnsVMS.RC_STATUS_CSUMFAIL;
            ...
            else
            {
                ...
                int nCmdIdByte = (int)RC_MsgDefnsVMS.MsgByte.MSG_BYTE_FIRST_CMD;        // Set the index of the first cmd identifier
                RC_MsgDefns.SuccessType eMsgGood;    // Pick up a success return value
                int nValidBytes = nMsgLength;
                do
                {
                    // Act on commands
                    if ( msg_buf[nCmdIdByte] < 0x80 )
                    {
                        Debug.Log("MsgCmdByte < 0x80");
                ...
                if ( RC_MsgDefns.SuccessType.DIS_OK != eMsgGood )
                {
                    ...
                    switch ( eMsgGood )
                    {
                        ...
                        case RC_MsgDefns.SuccessType.DIS_UNKNOWN_ITEM:
                            cStatusByte |= RC_MsgDefnsVMS.RC_STATUS_UNK_ITEM;
                            break;
                        case RC_MsgDefns.SuccessType.DIS_NOT_SUPPORTED:
                            cStatusByte |= RC_MsgDefnsVMS.RC_STATUS_NOT_SUPP;
                            break;
                        case RC_MsgDefns.SuccessType.DIS_NOT_VALID_STATE:
                            cStatusByte |= RC_MsgDefnsVMS.RC_STATUS_INV_STAT;
                            break;
                        case RC_MsgDefns.SuccessType.DIS_INVALID_ACCESS:
                            cStatusByte |= RC_MsgDefnsVMS.RC_STATUS_INV_ACCE;
                            break;
                        case RC_MsgDefns.SuccessType.DIS_INSUFFICIENT_BUFFER:
                            cStatusByte |= RC_MsgDefnsVMS.RC_STATUS_NO_BUFF;
                            break;
                        case RC_MsgDefns.SuccessType.DIS_OTHER_ERROR:
                            cStatusByte |= RC_MsgDefnsVMS.RC_STATUS_ERROR;
                            break;
                        case RC_MsgDefns.SuccessType.DIS_NOT_INIT:
                            cStatusByte |= RC_MsgDefnsVMS.RC_STATUS_NOT_INIT;
                            break;
                        case RC_MsgDefns.SuccessType.DIS_DEVICE_BUSY:
                            cStatusByte |= RC_MsgDefnsVMS.RC_STATUS_BUSY;
                            break;
                        case RC_MsgDefns.SuccessType.DIS_NOT_IN_CONTROL:
                            cStatusByte |= RC_MsgDefnsVMS.RC_STATUS_NOT_CNTL;
                            break;
                    }
                }

                // Complete the formation of the response
                RC_MsgHelpers.completeRC_Resp( cStatusByte, ref cReplyMsg, ref nReplyLength );
        ...
        public static void sendUnknownResp()
        {
            ...
            RC_MsgHelpers.formRC_RespHeader( 0, 0, ref cReplyMsg, ref nReplyLength );

            // Complete the formation of the response
            RC_MsgHelpers.completeRC_Resp( RC_MsgDefnsVMS.RC_STATUS_UNK_ITEM, ref cReplyMsg, ref nReplyLength );
        ...
        public class BackgroundPeriodicMsg
        {
            ...
            public BackgroundPeriodicMsg( cVMS_MsgHandler MsgHandler, UdpClient sock, IPEndPoint client, Dictionary<byte, PERIODIC_DEFN> dictPeriodic, int waitTime )
            {
        ...
        private RC_MsgDefns.SuccessType actionCfgWrite( byte[] pMsg, ref int pByteId, int nRxMsgBytes )
        {
            ...
            switch ( pMsg[pByteId] )
            {
                case RC_MsgDefnsVMS.DID_PeriodicMsgFreq:
                    {
                    ...
                    break;
                case RC_MsgDefnsVMS.DID_PeriodicMsgDefn:
                    {
                ...
                default:
                    {
                        Debug.Log("Default");
                    //TODO: Bit of a hack here for now, assume that gyro posn is deck posn!
                        if (pMsg[pByteId] == RC_MsgDefnsVMS.DID_PedGyroPosnAz)
                           {
                               pMsg[pByteId] = RC_MsgDefnsVMS.DID_PedDeckPosnAz;
                           }

                        if (pMsg[pByteId] == RC_MsgDefnsVMS.DID_PedGyroPosnEl)
                        {
                            pMsg[pByteId] = RC_MsgDefnsVMS.DID_PedDeckPosnEl;
                        }
        ...
        private RC_MsgDefns.SuccessType actionMsgDefnWrite( byte[] pDataStart, int nRxMsgBytes, int nByteId, ref int nBytesUsed )
        {
            ...
            foreach ( var iter in vecDataItems )
            {
                if ( pDataOut[0] < ( RC_MsgDefnsVMS.MAX_MSG_DATA_DEFNS - 1 ) )
                {
        ...
        private bool sendPeriodicMessage( UdpClient sock, IPEndPoint client )
        {
            ...
            RC_MsgHelpers.formRC_RespHeader( m_cSubSysId, m_cSubSysIndex, ref cStatusMsg, ref nMsgLength );
            ...
            cStatusByte = RC_MsgDefnsVMS.RC_STATUS_PERIODIC;

            // Complete the message
            RC_MsgHelpers.completeRC_Resp( cStatusByte, ref cStatusMsg, ref nMsgLength );
        ...
        private bool fillPeriodicMessage( byte[] pStatusMsg, int nBytesAvail, ref int nBytesUsed )
        {
            ...
            for ( int i = 0; i < pPeriodic.nMsgDefn[0]; i++ )
            {
                ...
                else if ( RC_MsgDefns.SuccessType.DIS_INSUFFICIENT_BUFFER == eRetVal )
                {
                    /// A real problem
                    Debug.Log( "Periodic Message Buffer Full" );
                    return false;
                }
            }
